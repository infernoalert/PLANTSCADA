...
| Area | Behavior |
|------|------------|
| **Clean** | Reads `input/EQPARAM.csv`, keeps rows whose `Equipment` **contains** the search string (case-insensitive substring), writes all matching columns to `output/cleanEqparam.csv` (no deduplication). |
| **TabViewr** | Uses a **second** search box; filters `Equipment` by **contains** (same as Clean); groups rows by **distinct** `Equipment` so different leaf tags (e.g. `PU4107` vs `PU4107_LT001`) export separately; builds sheets from `Tab_*_Title` / `Status_*_r*c*`; filenames use **leaf** slug: `leafSlug_v{v}_h{h}_titleSlug.csv`. Output CSVs have data rows only (no `c1`/`c2` header). |
